...
def main():
    ...
    for plot_idx, rank_idx in enumerate(ranks):
        ...
        if plot_idx == 2:
            ax.set_ylabel("Normalized surprisal", fontsize=20)
        if plot_idx == 3:
            ax2.set_ylabel("Proportion producing", fontsize=20)
    ...
    fig.text(0.5, 0.05, 'Normalized timeline', ha='center', fontsize=20)
    ...
    fig.legend(handles=handles1, loc='upper center', ncol=3, 
              bbox_to_anchor=(0.5, 0.98), fontsize=18, frameon=False)
    
    merged_path = os.path.join(args.out_dir, "merged_word_trajectories.png")
    fig.savefig(merged_path, bbox_inches="tight", dpi=150)
    plt.close(fig)
